...
def process_dy_sm_background():
    ...
    # 创建输出文件
    output_file = ROOT.TFile(os.path.join(output_dir, "sstest_templates_sig.root"), "RECREATE")
    ...
    output_file.Close()
    print("处理完成！输出文件: %s" % os.path.join(output_dir, "sstest_templates_sig.root"))
