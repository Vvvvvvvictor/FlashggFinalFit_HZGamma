...
def process_dy_sm_background():
    ...
    # 创建输出文件
    output_file = ROOT.TFile(os.path.join(output_dir, "sstest_templates.root"), "RECREATE")
    ...
    for cat in categories:
        hist_name = "%s_dy_sm" % cat
        histograms[cat] = ROOT.TH1F(hist_name, hist_name, 340, 95, 180)  # 340个bin，范围95-180 GeV
    
    # 定义要处理的文件模式
    file_patterns = [
        "DY0_*_output_1.root",
        "DYmix_*_output_1.root", 
        "DYfilter_*_output_1.root",
        "SM1_*_output.root"
    ]
    
    # 要排除的文件模式
    exclude_patterns = [
        "DY0_*_output.root"  # 排除不带_1的文件
    ]
    ...
    output_file.Close()
    print("处理完成！输出文件: %s" % os.path.join(output_dir, "sstest_templates.root"))

if __name__ == "__main__":
    process_dy_sm_background()