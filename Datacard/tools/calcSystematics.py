...
def getValueFromJson(row,uncertainties,sname):
  ...
  p = re.sub("_2016preVFP_%s"%decayMode,"",p)
...
def calcSystYields(_nominalDataName,_nominalDataContents,_inputWS,_systFactoryTypes,skipCOWCorr=True,proc="ggH",year='2016',ignoreWarnings=False):
  ...
  for s,f in _systFactoryTypes.iteritems():
	...
	systToSkip.append(s)
	print " --> [%s] Weight1 in nominal RooDataSet for systematic (%s) does not exist for (%s,%s). %s"%(errMessage,s,proc,year,errString)
	if not ignoreWarnings: sys.exit(1) 
    else:
      if s not in _nominalDataContents:
	systToSkip.append(s)
	print " --> [%s] Weight2 in nominal RooDataSet for systematic (%s) does not exist for (%s,%s). %s"%(errMessage,s,proc,year,errString)
	if not ignoreWarnings: sys.exit(1)
      ...
      # If asymmetric weights:
        ...
        if s in systToSkip: 
          ...
          # 2) if up weight is equal to down weight (=1) then set to nominal
            ...
            # print "centralWeightStr: %s, f_central: %s, f_up: %s, f_down: %s, w: %s, w_up: %s, w_down: %s"%(centralWeightStr,f_central,f_up,f_down,w,w_up,w_down)
            # w_up, w_down = w*f_up, w*f_down
  ...
  for s, f in _systFactoryTypes.iteritems():
    if f == "a_h":
      ...
      if( data_hist_up == None )|( data_hist_down == None ):
        ...
        if s == "energyErrShift": print(data_hist_up.sumEntries(), data_hist_down.sumEntries())
...
def compareYield(row,factoryType,sname,mode='default',mname=None):
  ...
  if( mode == 'default' )|( mode == 'ishape' ):
    ...
    if factoryType == "a_h":
      ...
      if sname == "energyErrShift": print("hh",midpoint_yield, [(row["%s_down_yield"%sname]),(row["%s_up_yield"%sname])])
