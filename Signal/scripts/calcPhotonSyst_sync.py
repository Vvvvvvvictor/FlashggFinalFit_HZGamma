...
for _flav in opt.flavs.split(","):
  _nominalDataName = "mcdata_Htozg_%s_cat_%s"%(flavMap_sync[_flav],catMap_sync[opt.cat])
  data = data.append({'proc':"all",'cat':opt.cat,'flav':_flav,'inputWSFile':_WSFileName,'nominalDataName':_nominalDataName}, ignore_index=True, sort=False)
...
for ir,r in data.iterrows():
  ...
  if r['flav'] == "ele": 
    _WSName = "WS_Htozg_el_cat_%s"%(catMap_sync[opt.cat])
  elif r['flav'] == "mu":
    _WSName = "WS_Htozg_mu_cat_%s"%(catMap_sync[opt.cat])
...
if not os.path.isdir("%s/output_%s/calcPhotonSyst"%(swd__,opt.ext)): os.system("mkdir %s/output_%s/calcPhotonSyst"%(swd__,opt.ext))
if not os.path.isdir("%s/output_%s/calcPhotonSyst/pkl"%(swd__,opt.ext)): os.system("mkdir %s/output_%s/calcPhotonSyst/pkl"%(swd__,opt.ext))
with open("%s/output_%s/calcPhotonSyst/pkl/%s.pkl"%(swd__,opt.ext,opt.cat),"wb") as f: pickle.dump(data,f) 
with open("%s/output_%s/calcPhotonSyst/pkl/%s.txt"%(swd__,opt.ext,opt.cat),"wb") as f: f.write(data.to_string(index=False))
print " --> Successfully saved photon systematics as pkl file: %s/output_%s/calcPhotonSyst/pkl/%s.pkl"%(swd__,opt.ext,opt.cat)
print(data)