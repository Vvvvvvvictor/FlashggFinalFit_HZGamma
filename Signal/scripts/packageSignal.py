...
# Save to file
if not os.path.isdir("%s/outdir_%s"%(opt.outputPath,opt.outputExt)): os.system("mkdir -p %s/outdir_%s"%(opt.outputPath,opt.outputExt))
if opt.mergeYears:
  print " --> Writing to: %s/outdir_%s/CMS-HGG_sigfit_%s_%s.root"%(opt.outputPath,opt.outputExt,opt.outputExt,opt.cat)
  f = ROOT.TFile("%s/outdir_%s/CMS-HGG_sigfit_%s_%s.root"%(opt.outputPath,opt.outputExt,opt.outputExt,opt.cat),"RECREATE")
else:
  print " --> Writing to: %s/outdir_%s/CMS-HGG_sigfit_%s_%s_%s.root"%(opt.outputPath,opt.outputExt,opt.outputExt,opt.cat,opt.year)
  f = ROOT.TFile("%s/outdir_%s/CMS-HGG_sigfit_%s_%s_%s.root"%(opt.outputPath,opt.outputExt,opt.outputExt,opt.cat,opt.year),"RECREATE")
